<!-- LOC_START --> Lines of Code: 4724 <!-- LOC_END -->
...
- **`decl.rs`** - 宣言情報．`Decl`（変数・関数宣言）と`MemberDecl`（構造体メンバ宣言）を定義
...
- **`utils.rs`** - ユーティリティ．`Span`（ソースコード位置情報）と`AlignUp`（アライメント計算）を定義
